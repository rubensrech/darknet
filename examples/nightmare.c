...
void optimize_picture(network *net, image orig, int max_layer, real scale, real rate, real thresh, int norm)
{
    ...
    real *imgDataReal = cast_array_float2real(im.data, im.w*im.h*im.c);
    copy_cpu(net->inputs, imgDataReal, 1, net->input, 1);
    ...
    cuda_pull_array(net->delta_gpu, deltaDataReal, im.w*im.h*im.c); // cuda_pull_array(net->delta_gpu, delta.data, im.w*im.h*im.c);
    ...
    for(e = 0; e < rounds; ++e){
        ...
        for(n = 0; n < iters; ++n){  
            ...
            if(reconstruct){
                ...
                int layer = max_layer + rand()%range - range/2;
                int octave = rand()%octaves;
                optimize_picture(net, im, layer, CAST(1/pow(1.33333333, octave)), rate, thresh, norm);
            }
        }
        fprintf(stderr, "done\n");
        if(0){
