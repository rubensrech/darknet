...
void train_lsd3(char *fcfg, char *fweight, char *gcfg, char *gweight, char *acfg, char *aweight, int clear)
{
    ...
    real *X = calloc(x_size, sizeof(real));
    ...
    while (get_current_batch(gnet) < gnet->max_batches) {
        ...
        for(j = 0; j < fnet->subdivisions; ++j){
            ...
            *gnet->seen += gnet->batch;
            ...
            real *feats = fnet->layers[fnet->n - 2].output_gpu;
            copy_gpu(y_size, feats, 1, fstate.truth, 1);

            forward_network_gpu(gnet, gstate);
            real *gen = gnet->layers[gnet->n-1].output_gpu;
...
*/
/*
        image sim = real_to_image(anet->w, anet->h, anet->c, style.X.vals[j]);
...
void train_pix2pix(char *cfg, char *weight, char *acfg, char *aweight, int clear)
{
    ...
    real *imerror = cuda_make_array(0, imlayer.outputs);
    real *ones_gpu = cuda_make_array(0, ay_size);
    ...
    while (get_current_batch(net) < net->max_batches) {
        ...
        for(j = 0; j < imgs; ++j){
            ...
            image yim = real_to_image(net->w, net->h, net->c, train.X.vals[j]);
            //rgb_to_yuv(yim);
        }
        time=clock();
        real gloss = 0;
        ...
        for(j = 0; j < net->subdivisions; ++j){
            ...
            */
            /*
            image origi = real_to_image(net->w, net->h, 3, pixs);
            image grayi = real_to_image(net->w, net->h, 3, graypixs);
...
void train_prog(char *cfg, char *weight, char *acfg, char *aweight, int clear, int display, char *train_images, int maxbatch)
{
    ...
    printf("Learning Rate: %g, Momentum: %g, Decay: %g\n", (float)gnet->learning_rate, (float)gnet->momentum, (float)gnet->decay);
    int imgs = gnet->batch*gnet->subdivisions;
    i = *gnet->seen/imgs;
    data train, buffer;
    ...
    while (get_current_batch(gnet) < maxbatch) {
        ...
        for (j = 0; j < gnet->subdivisions; ++j) {
            ...
            *gnet->seen += gnet->batch;
            ...
            scal_gpu(imlayer.outputs*imlayer.batch, CAST(1), imerror, 1);
            scal_gpu(imlayer.outputs*imlayer.batch, CAST(0), gnet->layers[gnet->n-1].delta_gpu, 1);

            axpy_gpu(imlayer.outputs*imlayer.batch, CAST(1), imerror, 1, gnet->layers[gnet->n-1].delta_gpu, 1);
...
#ifdef OPENCV
        if(display){
            image im = real_to_image(anet->w, anet->h, anet->c, gen.X.vals[0]);
            image im2 = real_to_image(anet->w, anet->h, anet->c, train.X.vals[0]);
            show_image(im, "gen", 1);
        ...
        printf("%d: adv: %f | adv_avg: %f, %f rate, %f seconds, %d images\n", i, (float)aloss, (float)aloss_avg, (float)get_current_rate(gnet), (float)sec(clock()-time), i*imgs);
        if(i%10000==0){
            char buff[256];
            sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i);
            save_weights(gnet, buff);
    ...
    while (get_current_batch(net) < net->max_batches) {
        ...
        for(j = 0; j < net->subdivisions; ++j){
            ...
            *net->seen += net->batch;
            ...
            scal_gpu(imlayer.outputs*imlayer.batch, CAST(1./100.), net->layers[net->n-1].delta_gpu, 1);

            scal_gpu(imlayer.outputs*imlayer.batch, CAST(1), imerror, 1);

            printf("realness %f\n", (float)cuda_mag_array(imerror, imlayer.outputs*imlayer.batch));
            printf("features %f\n", (float)cuda_mag_array(net->layers[net->n-1].delta_gpu, imlayer.outputs*imlayer.batch));

            axpy_gpu(imlayer.outputs*imlayer.batch, CAST(1), imerror, 1, net->layers[net->n-1].delta_gpu, 1);
...
#ifdef OPENCV
        if(display){
            image im = real_to_image(anet->w, anet->h, anet->c, gray.X.vals[0]);
            image im2 = real_to_image(anet->w, anet->h, anet->c, train.X.vals[0]);
            show_image(im, "gen", 1);
            show_image(im2, "train", 1);
        }
        ...
        printf("%d: gen: %f, adv: %f | gen_avg: %f, adv_avg: %f, %f rate, %f seconds, %d images\n", i, (float)gloss, (float)aloss, (float)gloss_avg, (float)aloss_avg, (float)get_current_rate(net), (float)sec(clock()-time), i*imgs);
        if(i%1000==0){
            char buff[256];
            sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i);
            save_weights(net, buff);
