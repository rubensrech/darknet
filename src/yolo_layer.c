...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes)
{
    ...
    l.biases = calloc(total*2, sizeof(real));
    if(mask) l.mask = mask;
    else{
...
void delta_yolo_class(real *output, real *delta, int index, int class, int classes, int stride, real *avg_cat)
{
    int n;
    if (delta[index]){
...
int get_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            int obj_index  = entry_index(l, 0, n*l.w*l.h + i, 4);
            real objectness = predictions[obj_index];
            ...
            for(j = 0; j < l.classes; ++j){
                ...
                real prob = objectness*predictions[class_index];
