...
layer make_lstm_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wo) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.output = (real*)calloc(outputs*batch*steps, sizeof(real));
    l.state = (real*)calloc(outputs*batch, sizeof(real));

    l.forward = forward_lstm_layer;
    l.update = update_lstm_layer;

    l.prev_state_cpu =  (real*)calloc(batch*outputs, sizeof(real));
    l.prev_cell_cpu =   (real*)calloc(batch*outputs, sizeof(real));
    l.cell_cpu =        (real*)calloc(batch*outputs*steps, sizeof(real));

    l.f_cpu =           (real*)calloc(batch*outputs, sizeof(real));
    l.i_cpu =           (real*)calloc(batch*outputs, sizeof(real));
    l.g_cpu =           (real*)calloc(batch*outputs, sizeof(real));
    l.o_cpu =           (real*)calloc(batch*outputs, sizeof(real));
    l.c_cpu =           (real*)calloc(batch*outputs, sizeof(real));
    l.h_cpu =           (real*)calloc(batch*outputs, sizeof(real));
    l.temp_cpu =        (real*)calloc(batch*outputs, sizeof(real));
    l.temp2_cpu =       (real*)calloc(batch*outputs, sizeof(real));
    l.temp3_cpu =       (real*)calloc(batch*outputs, sizeof(real));
    l.dc_cpu =          (real*)calloc(batch*outputs, sizeof(real));
    l.dh_cpu =          (real*)calloc(batch*outputs, sizeof(real));
...
#ifdef CUDNN
        cudnnSetTensor4dDescriptor(l.wf->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wf->out_c, l.wf->out_h, l.wf->out_w); 
        cudnnSetTensor4dDescriptor(l.wi->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wi->out_c, l.wi->out_h, l.wi->out_w); 
        cudnnSetTensor4dDescriptor(l.wg->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wg->out_c, l.wg->out_h, l.wg->out_w); 
        cudnnSetTensor4dDescriptor(l.wo->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wo->out_c, l.wo->out_h, l.wo->out_w); 

        cudnnSetTensor4dDescriptor(l.uf->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.uf->out_c, l.uf->out_h, l.uf->out_w); 
        cudnnSetTensor4dDescriptor(l.ui->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.ui->out_c, l.ui->out_h, l.ui->out_w); 
        cudnnSetTensor4dDescriptor(l.ug->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.ug->out_c, l.ug->out_h, l.ug->out_w); 
        cudnnSetTensor4dDescriptor(l.uo->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.uo->out_c, l.uo->out_h, l.uo->out_w); 
#endif
...
void forward_lstm_layer_gpu(layer l, network state)
{
    ...
    for (i = 0; i < l.steps; ++i) {
        ...
        axpy_gpu(l.outputs*l.batch, CAST(1), uf.output_gpu, 1, l.f_gpu, 1);

        copy_gpu(l.outputs*l.batch, wi.output_gpu, 1, l.i_gpu, 1);	
        axpy_gpu(l.outputs*l.batch, CAST(1), ui.output_gpu, 1, l.i_gpu, 1);	

        copy_gpu(l.outputs*l.batch, wg.output_gpu, 1, l.g_gpu, 1);	
        axpy_gpu(l.outputs*l.batch, CAST(1), ug.output_gpu, 1, l.g_gpu, 1);	

        copy_gpu(l.outputs*l.batch, wo.output_gpu, 1, l.o_gpu, 1);	
        axpy_gpu(l.outputs*l.batch, CAST(1), uo.output_gpu, 1, l.o_gpu, 1);	
        ...
        axpy_gpu(l.outputs*l.batch, CAST(1), l.temp_gpu, 1, l.c_gpu, 1);	
...
void backward_lstm_layer_gpu(layer l, network state)
{
    ...
    for (i = l.steps - 1; i >= 0; --i) {
        ...
        axpy_gpu(l.outputs*l.batch, CAST(1), l.dc_gpu, 1, l.temp2_gpu, 1);		
