...
layer make_gru_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wh) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.delta = (real*)calloc(outputs*batch*steps, sizeof(real));
    l.state = (real*)calloc(outputs*batch, sizeof(real));
    l.prev_state = (real*)calloc(outputs*batch, sizeof(real));
    l.forgot_state = (real*)calloc(outputs*batch, sizeof(real));
    l.forgot_delta = (real*)calloc(outputs*batch, sizeof(real));

    l.r_cpu = (real*)calloc(outputs*batch, sizeof(real));
    l.z_cpu = (real*)calloc(outputs*batch, sizeof(real));
    l.h_cpu = (real*)calloc(outputs*batch, sizeof(real));
...
#ifdef CUDNN
    cudnnSetTensor4dDescriptor(l.uz->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.uz->out_c, l.uz->out_h, l.uz->out_w); 
    cudnnSetTensor4dDescriptor(l.uh->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.uh->out_c, l.uh->out_h, l.uh->out_w); 
    cudnnSetTensor4dDescriptor(l.ur->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.ur->out_c, l.ur->out_h, l.ur->out_w); 
    cudnnSetTensor4dDescriptor(l.wz->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wz->out_c, l.wz->out_h, l.wz->out_w); 
    cudnnSetTensor4dDescriptor(l.wh->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wh->out_c, l.wh->out_h, l.wh->out_w); 
    cudnnSetTensor4dDescriptor(l.wr->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.wr->out_c, l.wr->out_h, l.wr->out_w); 
#endif
#endif
...
void forward_gru_layer_gpu(layer l, network net)
{
    ...
    for (i = 0; i < l.steps; ++i) {
        ...
        axpy_gpu(l.outputs*l.batch, CAST(1), wz.output_gpu, 1, l.z_gpu, 1);

        copy_gpu(l.outputs*l.batch, ur.output_gpu, 1, l.r_gpu, 1);
        axpy_gpu(l.outputs*l.batch, CAST(1), wr.output_gpu, 1, l.r_gpu, 1);
        ...
        axpy_gpu(l.outputs*l.batch, CAST(1), wh.output_gpu, 1, l.h_gpu, 1);
