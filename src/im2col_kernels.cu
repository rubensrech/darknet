...
// #if REAL != FLOAT
    ...
    void im2col_float_gpu(float *im,
        ...
        im2col_gpu_float_kernel<<<(num_kernels+BLOCK-1)/BLOCK,
            BLOCK>>>(
...
// #elif REAL != HALF
    __global__ void im2col_gpu_half_kernel(const int n, const half_device* data_im,
        ...
        for(; index < n; index += blockDim.x*gridDim.x){
            ...
            int h_in = h_out * stride - pad;
            int w_in = w_out * stride - pad;
            half_device* data_col_ptr = data_col;
            data_col_ptr += (channel_out * height_col + h_out) * width_col + w_out;
            const half_device* data_im_ptr = data_im;
            data_im_ptr += (channel_in * height + h_in) * width + w_in;
            for (int i = 0; i < ksize; ++i) {
                for (int j = 0; j < ksize; ++j) {
                    int h = h_in + i;
    ...
    void im2col_half_gpu(half *im,
        ...
        int width_col = (width + 2 * pad - ksize) / stride + 1;
        int num_kernels = channels * height_col * width_col;
        im2col_gpu_half_kernel<<<(num_kernels+BLOCK-1)/BLOCK,
            BLOCK>>>(
                    num_kernels, im, height, width, ksize, pad,
                    stride, height_col,
                    width_col, data_col);
