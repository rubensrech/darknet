...
layer make_region_layer(int batch, int w, int h, int n, int classes, int coords)
{
    ...
    l.biases = calloc(n*2, sizeof(real));
    l.bias_updates = calloc(n*2, sizeof(real));
...
box get_region_box(real *x, real *biases, int n, int index, int i, int j, int w, int h, int stride)
{
    box b;
...
void delta_region_mask(real *truth, real *x, int n, int index, real *delta, int stride, int scale)
{
    int i;
    for(i = 0; i < n; ++i){
...
void delta_region_class(real *output, real *delta, int index, int class, int classes, tree *hier, real scale, int stride, real *avg_cat, int tag)
{
    int i, n;
    if(hier){
        real pred = 1;
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        real scale =  l.output[obj_index];
                        l.delta[obj_index] = l.noobject_scale * (0 - l.output[obj_index]);
                        real p = scale*get_hierarchy_probability(l.output + class_index, l.softmax_tree, class, l.w*l.h);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
            ...
            if(l.softmax_tree){
                ...
                if(map){
                    for(j = 0; j < 200; ++j){
                        ...
                        real prob = scale*predictions[class_index];
