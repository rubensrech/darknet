...
layer make_rnn_layer(int batch, int inputs, int outputs, int steps, ACTIVATION activation, int batch_normalize, int adam)
{
    ...
    l.state = (real*)calloc(batch*outputs, sizeof(real));
    l.prev_state = (real*)calloc(batch*outputs, sizeof(real));
...
#ifdef CUDNN
    cudnnSetTensor4dDescriptor(l.input_layer->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.input_layer->out_c, l.input_layer->out_h, l.input_layer->out_w); 
    cudnnSetTensor4dDescriptor(l.self_layer->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.self_layer->out_c, l.self_layer->out_h, l.self_layer->out_w); 
    cudnnSetTensor4dDescriptor(l.output_layer->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_REAL, batch, l.output_layer->out_c, l.output_layer->out_h, l.output_layer->out_w); 
#endif
#endif
...
void forward_rnn_layer(layer l, network net)
{
    ...
    fill_cpu(l.outputs * l.batch * l.steps, CAST(0), self_layer.delta, 1);
    fill_cpu(l.outputs * l.batch * l.steps, CAST(0), input_layer.delta, 1);
    if(net.train) fill_cpu(l.outputs * l.batch, CAST(0), l.state, 1);
